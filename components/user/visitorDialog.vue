<template>
  <div class="customer-dialog">
    <el-dialog
      v-model="visible"
      :title="t('IdentityInfoManage.tip')"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      width="30%"
    >
      <div class="text-black">
        <span>{{ t("IdentityInfoManage.tipContentEmail") }}</span>
        <span>{{ t("IdentityInfoManage.tipContentPassword") }}</span>
      </div>
      <template #footer>
...
:deep(.el-dialog__footer) {
  @apply mt-11 flex items-center justify-center pt-0;
}
:deep(.el-button) {
